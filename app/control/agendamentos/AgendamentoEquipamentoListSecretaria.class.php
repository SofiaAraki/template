...
class AgendamentoEquipamentoListSecretaria extends TPage
{
    ...
    public function __construct()
    {
        ...
        $this->form->addQuickAction(_t('New'),  new TAction(array('AgendamentoEquipamentoForm', 'onEdit')), 'bs:plus-sign green');
    ...
    public function onPrint( $param )
    {
        try
        {
            ...
            if($loggedUnit == 2)
            {
                $criteria->add(new TFilter('unidade', 'IN', array(1,2,12))); //Acrescentei Connext
                //$criteria->add(new TFilter('unidade', '=', 2), TExpression::AND_OPERATOR);
            }
            else
            {
            ...
            //$criteria->add(new TFilter('inicio_ferias', '<=', $param['ate']));
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            if($loggedUnit == 2)
            {
                $criteria->add(new TFilter('unidade', 'IN', array(1,2, 12))); //MOSTRA AGENDAMENTOS FFCL, CNSC E CONNEXT
            }
            else
            {
                $criteria->add(new TFilter('unidade', '=', $loggedUnit));
