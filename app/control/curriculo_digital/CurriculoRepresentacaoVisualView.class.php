...
class CurriculoRepresentacaoVisualView extends TPage
{
    private $panel;
    ...
    public function __construct()
    {
        ...
        $this->panel->addHeaderActionLink('Publicar currículo', new TAction([$this, 'onQuestionPublicarCurriculo']), 'fa:check green');
    ...
    public function onLerDadosXml($param)
    {
        try
        {
            $id_curriculo = $param['id_curriculo_digital'];
    ...
    public function onGerarQrCode($param)
    {
        try
        {
            ...
            if(($curriculo->qrcode == NULL) OR ($curriculo->caminho_qrcode == NULL))
            {
                if($curriculo->codigo_validacao <> NULL)
                {                                               
                    //Gera URL única do curriculo - Produção
                    $url = 'https://academico.feituverava.com.br/consultacurriculo-codigo=' . $curriculo->codigo_validacao;
                    
                    //Gera URL única do curriculo - Homologação
                    //$url = 'https://192.168.10.107/academico/consultacurriculo-codigo=' . $curriculo->codigo_validacao;                                  
                    ...
                    $target_path = 'secretaria/curriculo_qrcode/curriculo_' . $curriculo->id;
                    $target_file = $target_path . '/' . 'qrcode_curriculo_' . $curriculo->id . '_' . $curriculo->codigo_validacao . '.jpg';
                    ...
                    $curriculo->url_curriculo = $url;
                    $curriculo->qrcode = 'qrcode_curriculo_' . $curriculo->id . '_' . $curriculo->codigo_validacao . '.jpg';
                    $curriculo->caminho_qrcode = $target_path;
                    
                    $curriculo->store();
                    
                    
                    //Adiciona o caminho do qrcode à variável de sessão
                    $informacoes_curriculo['object']->CaminhoQrCode = $target_file;
                    
                    TSession::setValue('informacoes_curriculo', $informacoes_curriculo);

                    $this->onVisualizarCurriculo($param);
                }
                else
                {
                    $action_voltar = new TAction('CurriculoList', 'onReload'); 
                    new TMessage('error', 'O código de validação precisa estar inserido no XML para que o currículo possa ser publicado', $action_voltar);
                    die;
                }
            }            
            else
            {
                //Adiciona o caminho do qrcode à variável de sessão
                $informacoes_curriculo['object']->CaminhoQrCode = $curriculo->caminho_qrcode . '/' . $curriculo->qrcode;
    ...
    public function onVisualizarCurriculo($param)
    {
        $informacoes_curriculo = TSession::getValue('informacoes_curriculo');
    ...
    public static function onQuestionPublicarCurriculo($param)
    {
        try
        {
            $informacoes_curriculo = TSession::getValue('informacoes_curriculo');
            ...
            $curriculo = new CurriculoDigital($informacoes_curriculo['object']->IdCurriculo);        
            
            if($curriculo)
            {
                //1º Verifica se a assinatura da IES está presente
                if($curriculo->status_assinatura_emissora == 0)
                {
                    $action1 = new TAction(array('CurriculoList', 'onReload'));                                       
                    new TMessage('error', 'O currículo deve ser assinado pela emissora antes de prosseguir', $action1);    
                    die;
                }
     
                
                //2º Verifica se é nova publicação
                if($curriculo->status_publicacao == 0)
                {
                    $action_publicar = new TAction(['CurriculoRepresentacaoVisualView', 'onPublicarCurriculo'], ['id_curriculo' => $curriculo->id]);                    
                    $action_voltar = new TAction(['CurriculoList', 'onReload']);
                ...
                else
                {
                    ...
                    new TMessage('info', 'Currículo já publicado', $action2);    
                    die;
                }
            }
            else
            {
                throw new Exception("Falha ao publicar currículo");
            }    
    ...
    public static function onPublicarCurriculo($param)
    {
        try
        {
            ...
            if($curriculo)
            {
                ...
                {
                    $curriculo->data_primeira_publicacao = date('Y-m-d');
                }
                else
                {
                    $curriculo->data_primeira_publicacao = $curriculo->data_primeira_publicacao;
                }
                
                $curriculo->status_publicacao = 1;
                $curriculo->data_publicacao = date('Y-m-d');
                $curriculo->system_user_id = TSession::getValue('userid');
                $curriculo->data_reg = date('Y-m-d H:i:s');
                $curriculo->store();                
            }
            else
            {
            ...
            new TMessage('info', 'Currículo publicado com sucesso!');
