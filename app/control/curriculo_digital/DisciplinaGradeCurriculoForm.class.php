...
//Este formulário vai salvar as disciplinas que fazem parte da grade no currículo

class DisciplinaGradeCurriculoForm extends TPage
{
    protected $form; 
    private $cod_disciplina_grade_etapa;
    ...
    public function __construct( $param )
    {
        ...
        //Filtra somente as disciplinas cadastradas na grade para exibir no seekbutton
        try
        {
            ...
            //Pega os códigos das etapas da grade correspondente
            TTransaction::open('dados_fei');
            
            $etapas_grade = FiGradeEtapa::where('CodGradecurso', '=', $curriculo->cod_grade)
                                        ->orderBy('Etapa')
                                        ->load();
            
            foreach($etapas_grade as $etapa_grade)
            {
                $codigos_grade_etapas[] = $etapa_grade->CodGradeEtapa;
            }
            
            //Critério para trazer o código das disciplinas que fazem parte das etapas da grade    
            $criteria1 = new TCriteria;            
            $criteria1->add(new TFilter('CodGradeEtapa', 'IN', $codigos_grade_etapas));
            $criteria1->setProperty('order', 'CodGradeEtapa, CargaHorariaTotal'); 
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_CurriculoDisciplina');
        ...
        $this->cod_disciplina_grade_etapa = new TDBSeekButton('cod_disciplina_grade_etapa', 'dados_fei', 'form_CurriculoDisciplina', 'FiGradeDisciplinaEtapa', 'CodDisciplina');                    
        ...
        $ch_genesi = new TEntry('ch_genesi');
        
        
        //Preenche campos com informações sobre a disciplina
        $this->cod_disciplina_grade_etapa->setExitAction(new TAction(array($this, 'onExitDisciplinaGradeEtapa')));
        ...
        $row = $this->form->addFields( [ new TLabel('Tipo'), $tipo ],
                                       [ new TLabel('Buscar na grade'), $this->cod_disciplina_grade_etapa ],
                                       [ new TLabel('Cód.'), $cod_disciplina ],
                                       [ new TLabel('Nome'), $nome ],
                                       [ new TLabel('Etapa'), $etapa ]);
        $row->layout = ['col-sm-3', 'col-sm-2', 'col-sm-2', 'col-sm-4', 'col-sm-1'];
        
        $row = $this->form->addFields( [ new TLabel('CH'), $ch_genesi ],
                                       [ new TLabel('Lançamento da CH no Genesi'), $opcao_carga_horaria ],
                                       ...
                                       [ new TLabel('Cód. no currículo'), $cod_disciplina_curriculo ]);
        $row->layout = ['col-sm-1', 'col-sm-3', 'col-sm-2', 'col-sm-2', 'col-sm-2', 'col-sm-2'];
        
        $row = $this->form->addFields( [ new TLabel('Ementa (usar somente texto, sem formatação, uso de tópicos e afins)'), $ementa ] );
        ...
        //PRÉ-REQUISITOS
        $label1 = new TLabel('Pré-requisitos', '#285097', 12, 'b', '<br>');
        $label1->style='text-align:left;border-bottom:1px solid #c0c0c0;width:100%';
        $this->form->addContent( [$label1] );
        
        
        //Adiciona checklist com as disciplinas da mesma grade já cadastradas
        try
        {
            TTransaction::open('Felabs_DB');
            ...
            $criteria2->add(new TFilter('opcao_disciplina', '=', 'Grade'));
            $criteria2->setProperty('order', 'etapa');
            
            $disciplinas = CurriculoDisciplina::getObjects($criteria2); 
        ...
        $pre_requisito_list = new TCheckList('pre_requisito_list');
        
        $pre_requisito_list->addColumn('id', 'ID', 'center', '10%')->setVisibility(false);
        $pre_requisito_list->addColumn('cod_disciplina_grade_etapa', 'Cód. Disciplina Etapa/Grade', 'center', '10%')->setVisibility(false);
        $pre_requisito_list->addColumn('etapa', 'Etapa', 'center', '10%');
        $pre_requisito_list->addColumn('cod_disciplina', 'Cód.', 'left', '10%');
        $pre_requisito_list->addColumn('nome', 'Disciplina', 'left', '50%');
        
        $pre_requisito_list->setHeight(250);
        $pre_requisito_list->makeScrollable();        
                                
        $input_search = new TEntry('search');
        $input_search->placeholder = _t('Search');
        $input_search->setSize('100%');
        $pre_requisito_list->enableSearch($input_search, 'cod_disciplina, nome');
                
        $hbox = new THBox;
        $hbox->style = 'border-bottom: 1px solid gray;padding-bottom:10px';
        $hbox->add(new TLabel('Grade Curricular'));
        $hbox->add($input_search)->style = 'float:right;width:30%;';
        
        $pre_requisito_list->addItems($disciplinas);
        

        $this->form->addFields( [ new TLabel("Unidade(s) curricular(es) que deve(m) ser <b>OBRIGATORIAMENTE</b> cursada(s) pelo aluno <b>ANTES</b> de 
        integralizar a unidade atual. Caso a disciplina pré-requisitada não esteja disponível na lista, verifique se a mesma já foi cadastrada no currículo") ] );
        $this->form->addContent( [$hbox] );
        $this->form->addFields( [$pre_requisito_list] );
        ...
        //ETIQUETAS
        $label2 = new TLabel('Etiquetas', '#285097', 12, 'b', '<br>');
        ...
        try
        {
            ...
            $criteria3->add(new TFilter('dados_curso_id', '=', $curso->id));
            
            $area_formacao = AreaFormacao::getObjects($criteria3);
        ...
        if($area_formacao)
        {
            $label3 = new TLabel('Áreas', '#285097', 12, 'b', '<br>');
            $label3->style='text-align:left;border-bottom:1px solid #c0c0c0;width:100%';
            $this->form->addContent( [$label3] );
        }
            
        $areas = new TDBCheckGroup('areas', 'Felabs_DB', 'AreaFormacao', 'id', 'nome', 'nome', $criteria3);
        ...
        $cod_disciplina->addValidation('Cód.', new TRequiredValidator);
        $this->cod_disciplina_grade_etapa->addValidation('Buscar na grade', new TRequiredValidator);
        ...
        $opcao_carga_horaria->addValidation('Lançamento da CH no Genesi', new TRequiredValidator);
        ...
        $this->cod_disciplina_grade_etapa->setCriteria($criteria1);
        $this->cod_disciplina_grade_etapa->setDisplayLabel('Disciplina');
        $this->cod_disciplina_grade_etapa->setDisplayMask('Cód. ({CodDisciplina}) - {fi_disciplina->NomeOficial}');
        $cod_disciplina->setEditable(FALSE);
        $nome->setEditable(FALSE);
        $etapa->setEditable(FALSE);
        $ch_genesi->setEditable(FALSE);
        $opcao_carga_horaria->setLayout('horizontal');
        $opcao_carga_horaria->setSize(85);
        $ch_hora_aula->setEditable(FALSE);
        $ch_hora_relogio->setEditable(FALSE);
        $opcao_disciplina->setValue('Grade');
    ...
    public static function onExitDisciplinaGradeEtapa($param)
    {
        try
        {
            TTransaction::open('dados_fei');
            
            $cod_disciplina_grade_etapa = $param['cod_disciplina_grade_etapa']; 
            
            $disciplina_grade_etapa = new FiGradeDisciplinaEtapa($cod_disciplina_grade_etapa);
            
            $grade_etapa = new FiGradeEtapa($disciplina_grade_etapa->CodGradeEtapa); 
            
            $grade_curso = new FiGradeCurso($grade_etapa->CodGradecurso);

            $cod_curriculo = $param['cod_curriculo'];
            
            
            $obj = new StdClass;
            $obj->cod_disciplina = $disciplina_grade_etapa->CodDisciplina;
            $obj->nome = $disciplina_grade_etapa->fi_disciplina->NomeOficial;
            $obj->etapa = $grade_etapa->Etapa;
            $obj->ch_genesi = $disciplina_grade_etapa->CargaHorariaTotal;
             
            //O código da unidade no currículo é formado pelo código do currículo (cód.curso + "." + cód. grade) + "-" + código da disciplina
            $obj->cod_disciplina_curriculo = $cod_curriculo . '-' . $disciplina_grade_etapa->CodDisciplina;                     
            
            
            //Limpa os campos da ch para garantir que serão atualizados caso troque de disciplina e ela tenha ch diferente
            ...
            TForm::sendData('form_CurriculoDisciplina', $obj);
    ...
    {
        $ch_genesi = $param['ch_genesi'];
        $opcao_carga_horaria = $param['opcao_carga_horaria'];
            
        if((!empty($ch_genesi)) AND (!empty($opcao_carga_horaria)))
        {    
            $obj = new StdClass;
               
            if($opcao_carga_horaria == "Hora/Aula")
            {
                $obj->ch_hora_aula = (int) $ch_genesi;
                $obj->ch_hora_relogio = ($ch_genesi * 50)/60;
                $obj->ch_hora_relogio = (int) round($obj->ch_hora_relogio);

                $obj->ch_hora_aula_extensao = '';
                $obj->ch_hora_relogio_extensao = '';
                
                TForm::sendData('form_CurriculoDisciplina', $obj, FALSE, FALSE);
            ...
            {
                $obj->ch_hora_relogio = $ch_genesi;
                $obj->ch_hora_aula = ($ch_genesi * 60)/50;
        ...
        try
        {
            ...
            if(in_array($id_etiqueta_extensao, $etiquetas_selecionadas))
            {
                if(!empty($opcao_carga_horaria))
                {
                    ...
                    if($opcao_carga_horaria == "Hora/Aula")
                    {      
                        TEntry::enableField('form_CurriculoDisciplina', 'ch_hora_aula_extensao');                                                
                        TEntry::disableField('form_CurriculoDisciplina', 'ch_hora_relogio_extensao');
                    }
                    elseif($opcao_carga_horaria == "Hora/Relógio")
                    {
                        TEntry::enableField('form_CurriculoDisciplina', 'ch_hora_relogio_extensao');
                        TEntry::disableField('form_CurriculoDisciplina', 'ch_hora_aula_extensao');
                    }
                    else
                    {
                        TEntry::clearField('form_CurriculoDisciplina', 'ch_hora_aula_extensao');
                        TEntry::clearField('form_CurriculoDisciplina', 'ch_hora_relogio_extensao');
                ...
                {
                    TCheckGroup::clearField('form_CurriculoDisciplina', 'etiqueta');
    ...
    public function onSave( $param )
    {
        try
        {
            ...
            if(empty($data->id))
            {
                ...
                $criteria2->add(new TFilter('cod_disciplina_grade_etapa', '=', $object->cod_disciplina_grade_etapa)); 
            ...
            //Controle campos condicionais - Pré-requisitos
            $old_pre_requisitos = CurriculoDisciplinaRequisitada::where('curriculo_disciplina_dependente_id', '=', $object->id)->load();
            
            if($data->pre_requisito_list)
            {
                foreach($data->pre_requisito_list as $disciplina_requisitada)
                {
                    $verifica_disciplina = CurriculoDisciplinaRequisitada::where('curriculo_disciplina_dependente_id', '=', $object->id)
                                                                         ->where('curriculo_disciplina_requisitada_id', '=', $disciplina_requisitada)
                                                                         ->load();
                
                    //Se não encontrou registro, é novo                                                     
                    if(empty($verifica_disciplina))
                    {
                        $curriculo_disciplina_requisitada = new CurriculoDisciplinaRequisitada;
                    }
                    
                    //Se encontrou registro, é porque a disciplina requisitada já havia sido selecionada anteriormente
                    else
                    {
                        $curriculo_disciplina_requisitada = CurriculoDisciplinaRequisitada::find($verifica_disciplina[0]->id);
                    }
                    
                    $curriculo_disciplina_requisitada->curriculo_disciplina_dependente_id = $object->id;
                   	$curriculo_disciplina_requisitada->curriculo_disciplina_requisitada_id = $disciplina_requisitada;
                   	$curriculo_disciplina_requisitada->store();
                   	
                    $pre_requisitos[] = $curriculo_disciplina_requisitada->id;
                }
            }
            
            if($old_pre_requisitos)
            {
                foreach($old_pre_requisitos as $old_pre_requisito)
                {
                    if (!in_array($old_pre_requisito->id, $pre_requisitos))
                    {
                        $old_pre_requisito->delete();
                    }
                }
            }
            ...
            if($datagrid_origem == "CurriculoList")
            {
                ...
                TApplication::loadPage('DisciplinaGradeCurriculoForm', 'onLoad', ['curriculo_id' => $object->curriculo_id]);    
            }
            else
            {
        ...
        catch (Exception $e) 
        {
            ...
            //Se estiver editando registro e cair na exceção, mantém o mesmo código de disciplina
            if(!empty($param['id']))
            {
                $this->cod_disciplina_grade_etapa->setEditable(FALSE);
            }
    ...
    public function onEdit( $param )
    {
        try
        {
            if (isset($param['key']))
            {
                ...
                $this->cod_disciplina_grade_etapa->setEditable(FALSE);
                $object->ch_hora_relogio = (int) $object->ch_hora_relogio;
                
                
                //Disciplinas pré-requisitadas
                $pre_requisitos = $object->getCurriculoDisciplinaRequisitada();
                
                if($pre_requisitos)
                {
                    foreach($pre_requisitos as $pre_requisito)
                    {
                        $pre_requisito_list[] = $pre_requisito->curriculo_disciplina_requisitada_id;
                    }
                }
                
                $object->pre_requisito_list = $pre_requisito_list;
                ...
                $object->ch_hora_relogio_extensao = (int) $ch_hora_relogio_extensao;
                $object->ch_hora_aula_extensao = $ch_hora_aula_extensao;                  
                ...
                $disciplina_grade_etapa = new FiGradeDisciplinaEtapa($object->cod_disciplina_grade_etapa);
