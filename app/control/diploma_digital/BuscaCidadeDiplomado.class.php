...
class BuscaCidadeDiplomado extends TWindow
{
    private $form;
    private $datagrid;
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_BuscaCidadeDiplomado');
        ...
        $column_CODCIDADE_INEP = new TDataGridColumn('CODCIDADE_INEP', 'Código IBGE', 'center', 100);
        $column_Nome = new TDataGridColumn('Nome', 'Nome do município', 'left', 200);
        $column_Uf = new TDataGridColumn('Uf', 'UF', 'center', 50);
        $column_CEPINICIAL = new TDataGridColumn('fi_cidades_inep->CEPINICIAL', 'CEP', 'center', 100);
    ...
    public static function onSelect($param)
    {
        try
        {
            ...
            $send = new StdClass;
            $send->naturalidade_cod_municipio = $object->CODCIDADE_INEP;
            $send->naturalidade_nome_municipio = $object->Nome;
            $send->naturalidade_uf = $object->Uf;
        ...
        {
            $send = new StdClass;
            $send->naturalidade_cod_municipio = '';
            $send->naturalidade_nome_municipio = '';
            $send->naturalidade_uf = '';
            TForm::sendData('form_DiplomaDigitalDiplomado', $send);
