...
class EstagioSecretariaList extends TPage
{
    private $form; 
    private $datagrid; 
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('<h4>Estágios</h4>');
        ...
        $column_cod_prof_responsavel = new TDataGridColumn('cod_prof_responsavel', 'Responsável pela aprovação', 'center');
        ...
        $action_admin = new TDataGridAction(['EstagioAdminForm', 'onEdit'], ['id'=>'{id}']);
        $action_visualizar = new TDataGridAction(['EstagioAnalisadoProfessorFormView', 'onEdit'], ['id'=>'{id}']);        
        ...
        $action_admin->setLabel('Editar estágio');
        $action_admin->setImage('fas: fa-user-lock red');
        $action_admin->setDisplayCondition( array($this, 'displayColumnEditarEstagioAdmin') );
    ...
    public function displayColumnEditarEstagioAdmin($object)
    {
        $grupo_admin = 1;
    ...
    /*public static function onConverterPDF($param)
    {
        try
        {
            ...
            if($estagio->status_pdfa == 0)
            {            
                //Ghostscript usa o caminho absoluto
                $caminho_pdf = $estagio->caminho_arquivo . '/' . $estagio->arquivo;
                ...
                if(file_exists($caminho_pdfa2))
                {
                    ...
                    $estagio->arquivo = substr($estagio->arquivo, 0, -4) . '_A2b.pdf';
                    $estagio->status_pdfa = 1;
                    $estagio->system_user_id = TSession::getValue('userid');
                    $estagio->data_reg = date('Y-m-d H:i:s');  
    ...
    {
        //Função não concluída, não colocar em funcionamento
        ...
        try
        {
            ...
            $caminho_absoluto_estagio = realpath($estagio->caminho_arquivo . '/' . $estagio->arquivo);  
            $caminho_absoluto_diretorio = dirname(realpath($estagio->caminho_arquivo . '/' . $estagio->arquivo)) . "\\"; 
            
            
            //Esta função faz assinatura de forma "anexada" e não no formato Pades (embutida), como pede o MEC
            if($estagio->status_estagio == "Aprovado" AND $estagio->status_pdfa == 1)
            {
                ...
                $param->setPdf(file_get_contents($caminho_absoluto_estagio));
                $param->setPassword('--------');
                ...
                file_put_contents($caminho_absoluto_estagio, $fileSigned);
                
                ---TERMINAR---
            }
            else
            {
                new TMessage('error', 'O estágio precisa ter sido aprovado pelo professor responsável e convertido em PDF/A para ser assinado');
            }
    ...
    public function onSearch()
    {
        ...
        /*if (isset($data->cod_prof_responsavel) AND ($data->cod_prof_responsavel)) {
            $filter = new TFilter('(SELECT name from system_users WHERE systemuser_codlegado=estagio.cod_prof_responsavel)', 'like', "%{$data->cod_prof_responsavel}%");
        ...
        if ($data->cod_prof_responsavel) {
            $filter = new TFilter('cod_prof_responsavel','in',"(SELECT su.systemuser_codlegado
                                    ...
                                    join estagio e
                                    on su.systemuser_codlegado = e.cod_prof_responsavel 
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            //Exibe só os estágios da unidade logada
            TTransaction::open('Felabs_DB');      
    ...
    public function displayColumnDelete($object)
    {
        try
        {
            ...
            //if(($object->status_estagio == "Aguardando aprovação" AND $object->system_user_id == $user_id) OR (($object->status_estagio == "Aguardando aprovação") AND (in_array($grupo_admin, $user_groups))))
