...
class HistoricoRepresentacaoVisualView extends TPage
{
    private $panel;
    ...
    public function __construct()
    {
        ...
        $this->panel->addHeaderActionLink('Voltar', new TAction([$this, 'onBackList']), 'fa:arrow-left black');
        $this->panel->addHeaderActionLink('Salvar PDF', new TAction([$this, 'onDownload']), 'far:file-pdf red');
        $this->panel->addHeaderActionLink('Publicar histórico e notificar aluno', new TAction([$this, 'onQuestionPublicarHistorico']), 'fa:check green');
        $this->panel->style = 'min-height:600px';    
    ...
    {
        $formulario_origem = TSession::getValue('formulario_origem');
    ...
    public function onLerDadosXml($param)
    {
        ...
        try
        {
            $id_historico = $param['id_historico_digital'];
            ...
            $target_file = $historico_digital->caminho_arquivo . '/' . $historico_digital->arquivo;
    ...
    public function onFormatarDados($param)
    {
        try
        {
            ...
            TSession::setValue('dados_gerais', array('IdHistorico' => $historico_digital->id,
            ...
            if($historico_atividades)
            {
                ...
                $view_historico = new VwDadoshistoricoaluno($historico_digital->historico_genesi_id);
            ...
            foreach($dados_atividades as $dados_atividade)
            {
                ...
                $entrada_atividade = AtividadeComplementar::where('cod_aluno', '=', $historico_digital->cod_aluno)
            ...
            $criteria3->add(new TFilter('historico_digital_id', '=', $historico_digital->id)); 
            ...
            foreach($dados_estagios as $dados_estagio)
            {
                ...
                $entrada_estagio = Estagio::where('cod_aluno', '=', $historico_digital->cod_aluno)
                                          ->where('cod_curso', '=', $historico_digital->cod_curso)
    ...
    public function onGerarQrCode($param)
    {
        ...
        try
        {
            ...
            if(($historico->qrcode == NULL) OR ($historico->caminho_qrcode == NULL))
            {
                if($historico->codigo_validacao <> NULL)
                {                                               
                    //Gera URL única do histórico - Produção
                    $url = 'https://academico.feituverava.com.br/consultahistorico-codigo=' . $historico->codigo_validacao;
             
                    //Gera URL única do histórico - Homologação
                    //$url = 'https://192.168.10.107/academico/consultahistorico-codigo=' . $historico->codigo_validacao;                  
                    ...
                    $target_path = 'secretaria/historico_qrcode/historico_' . $historico->id;
                    $target_file = $target_path . '/' . 'qrcode_historico_' . $historico->id . '_' . $historico->codigo_validacao . '.jpg';
                    ...
                    if (file_exists($target_path))
                    {
                        ...
                        //Gerar e salvar a imagem do QR no servidor
                        $qrcode->render($url, $target_file);      
                    }
                    
                    $historico->url_historico = $url;
                    $historico->qrcode = 'qrcode_historico_' . $historico->id . '_' . $historico->codigo_validacao . '.jpg';
                    $historico->caminho_qrcode = $target_path;

                    $historico->store();
                    
                    
                    //Adiciona o caminho do qrcode à variável de sessão
                    $dados_gerais['CaminhoQrCode'] = $target_file;
                    
                    TSession::setValue('dados_gerais', $dados_gerais);

                    $this->onVisualizarHistorico($param);
                }
                else
                {
                    $action_voltar = new TAction(array($formulario_origem, 'onReload')); 
                    new TMessage('error', 'O código de validação precisa estar inserido no XML para que o histórico possa ser publicado', $action_voltar);
                    die;
                }
            }            
            else
            {
                //Adiciona o caminho do qrcode à variável de sessão
                $dados_gerais['CaminhoQrCode'] = $historico->caminho_qrcode . '/' . $historico->qrcode;
    ...
    public function onVisualizarHistorico($param)
    {
        ...
        if ($dados_atividades !== null) 
        {
            ...
            {
                // Se a carga horária obrigatória das atividades for nula, desabilita a seção
                $html_impressao->disableSection('ch-obrigatoria-atividades');
            }
        } 
        else 
        {
            // Desabilita seções relacionadas às atividades se $dados_atividades for nulo
            $html_impressao->disableSection('tabela-atividades');
            $html_impressao->disableSection('atividades');
            $html_impressao->disableSection('ch_obrigatoria_atividades');
        }
                
        if ($atividades_ch_integralizada !== null) 
        {
            $html_impressao->enableSection('atividades_ch_integralizada', $atividades_ch_integralizada);
        } 
        else 
        {
            $html_impressao->disableSection('atividades_ch_integralizada');
        }
        ...
        if($dados_estagios !== null)
        {
            ...
            {
                // Se a carga horária obrigatória das atividades for nula, desabilita a seção
                $html_impressao->disableSection('ch_obrigatoria_estagios');
            }
        }
        else 
        {
            // Desabilita seções relacionadas às atividades se $dados_atividades for nulo
            $html_impressao->disableSection('tabela-estagios');
            $html_impressao->disableSection('estagios');
            $html_impressao->disableSection('ch_obrigatoria_estagios');
        }    
        ...
        if ($estagios_ch_integralizada !== null) 
        {
            $html_impressao->enableSection('estagios_ch_integralizada', $estagios_ch_integralizada);
        } 
        else 
        {
            $html_impressao->disableSection('estagios_ch_integralizada');
        }
    ...
    public static function onDownload()
    {
        $html_impressao = new THtmlRenderer('app/resources/HistoricoDigital.html');
        ...
        $html_impressao->enableSection('disciplinas_ch_integralizada', $disciplinas_ch_integralizada, TRUE);
         ...
         ///Alterações realizadas no dia 13/06/2024 para atender aos históricos parciais e trasferências no qual não é obrigatório constar atividades complementares e estágio. - Pamella Scapim
        //Atividades
        $dados_atividades = TSession::getValue('atividades');
    ...
    public static function onQuestionPublicarHistorico($param)
    {
        try
        {
            $dados_gerais = TSession::getValue('dados_gerais');
            ...
            $historico = new HistoricoDigital($dados_gerais['IdHistorico']);        
            
            if($historico)
            {
                //1º Verifica se as assinaturas estão presentes
                if($historico->tipo_historico <> "Parcial" AND $historico->tipo_historico <> "2ª via parcial")
                {
                    //Se não contiver as assinaturas da secretária e da emissora, barra
                    if($historico->status_assinatura_secretaria == 0 OR $historico->status_assinatura_emissora == 0)
                    {
                        $action1 = new TAction(array($formulario_origem, 'onReload'));                                       
                        new TMessage('error', 'O histórico deve ser assinado pela secretária e pela emissora antes de prosseguir', $action1);    
                        die;
                    }
                }
                else
                {
                    //Se não contiver a assinatura da emissora, barra
                    if($historico->status_assinatura_emissora == 0)
                    {
                        $action2 = new TAction(array($formulario_origem, 'onReload'));                                       
                        new TMessage('error', 'O histórico deve ser assinado pela emissora antes de prosseguir', $action2);    
                        die;
                    }
                }     
                
                //2º Verifica se é nova publicação ou reenvio dos dados de acesso
                if($historico->status_publicacao == 0)
                {
                    $action_publicar = new TAction(['HistoricoRepresentacaoVisualView', 'onPublicarHistorico'], ['id_historico' => $historico->id]);                    
                ...
                if($historico->status_publicacao == 1)
                {
                    $param['id_historico'] = $historico->id;
                    self::onNotificarAluno($param);
                }
            }
            else
            {
                throw new Exception("Falha ao publicar histórico");
            }    
    ...
    public static function onPublicarHistorico($param)
    {
        try
        {
            $id_historico = $param['id_historico'];            
            ...
            if($historico)
            {
                $historico->status_publicacao = 1;
                $historico->data_publicacao = date('Y-m-d H:i:s');
                $historico->system_user_id = TSession::getValue('userid');
                $historico->data_reg = date('Y-m-d H:i:s');
                $historico->store();                
            }
            else
            {
            ...
            new TMessage('info', 'Histórico publicado com sucesso!');
    ...
    public static function onNotificarAluno($param)
    {
        ...
        {      
            ...
            $form->addFields( [ new TLabel('E-mail para envio das instruções de acesso à página de consulta do histórico') ] );
            $form->addFields( [ $email ] );
            
            $form->addAction('Notificar aluno', new TAction([__CLASS__, 'onEnviarNotificacao'], ['id_historico' => $id_historico]), 'far:check-circle blue');
            ...
            $historico_digital = new HistoricoDigital($id_historico);
            $diplomado = new DiplomaDigitalDiplomado($historico_digital->dados_diplomado_id);            
    ...
    public static function onEnviarNotificacao($param)
    {
        try
        {
                                    ...
                                    <table align='center' border='0' cellpadding='0' cellspacing='0' width='600' style='border-collapse: collapse;'>
                                        <tr>
                                            <td align='center' style='padding-top: 0px; padding-right: 0px; padding-bottom: 0px; padding-left: 0px;'>
                                                <img src='http://academico.feituverava.com.br/app/images/cabecalho_email_historico.png' alt='Histórico Digital FE' width='600' style='display: block;'/>
                                            </td>
                                        </tr>
                                        <tr>
                                                ...
                                                <table border='0' cellpadding='0' cellspacing='0' width='100%'>
                                                    ...
                                                    <tr>
                                                        <td style='padding: 20px 0 30px 0; color: #153643; font-family: Arial, sans-serif; font-size: 16px; line-height: 25px; text-align: justify;'>
                                                            Atendendo à normatização do MEC, o histórico passa a ser um documento com existência, emissão e armazenamento integralmente digitais.<br><br> 
                                                            ...
                                                            o que torna o processo de emissão muito mais rápido, prático e seguro, além da garantia de validade jurídica.<br><br> 
                                                            Para acessar seu histórico digital, siga estas instruções:<br><br>
                                                            1) Acesse nosso Portal Acadêmico através deste link: <a href='https://academico.feituverava.com.br/consultahistorico'>https://academico.feituverava.com.br/consultahistorico</a><br><br>
                                                            2) Durante o processo, será solicitado que você insira o código de validação do seu histórico. Utilize o seguinte código de validação: <b>$historico_digital->codigo_validacao</b>
                                                            <br><br> 
                                        ...
                                        <tr>
                                            <td align='center' style='padding-top: 5px; padding-right: 0px; padding-bottom: 5px; padding-left: 0px;'>
                                                <img src='http://academico.feituverava.com.br/app/images/fundo-intranet-logo.png' alt='Histórico Digital FE' width='400' style='display: block;'/>
                                            </td>
                                        </tr>
                                    </table>
                                </td>
            ...
            $mail->setSubject('Histórico emitido');
            ...
            //Email para acompanhar o processo
            $mail = new TMail;
            $mail->setFrom($prefs['mail_from'], 'Fundação Educacional de Ituverava');
            $mail->setSubject('Histórico emitido');
            $mail->setHtmlBody($message);
            $mail->addAddress('pamella@feituverava.com.br', 'Pamella Scapim');
            $mail->SetUseSmtp();
            $mail->SetSmtpHost($prefs['smtp_host'], $prefs['smtp_port']);
            $mail->SetSmtpUser($prefs['smtp_user'], $prefs['smtp_pass']);
            $mail->send();
