...
class HistoricoVisualizacaoPublicaFormView extends TPage
{
    private $panel;
    private $notebook;
    ...
    /*Se digitar a URL única direto ou tentar acessar pelo QrCode, vai para 'onSetDadosHistorico'. 
    ...
    public function onVerificaCodigo($param)
    {
        try
        {
            ...
            if(($codigo == NULL) OR (empty($codigo)))
            {
                ...
                new TMessage('error', "Não foi possível localizar o histórico. Verifique o código de validação e tente novamente.", $action_recarregar1);
                die;
            }
            ...
            {
                //Traz o histórico que corresponde ao código de validação (deve ser único)
                $count = HistoricoDigital::where('codigo_validacao', "=", $codigo)
                                         ->where('dados_emissora_id', 'IN', '(SELECT id FROM dados_emissora WHERE codigo_mec = ' . $codigo_mec . ')')
                                         ->count();
                
                if($count == 1)
                {
                    $dados_historico = HistoricoDigital::where('codigo_validacao', "=", $codigo)
                                                       ->where('dados_emissora_id', 'IN', '(SELECT id FROM dados_emissora WHERE codigo_mec = ' . $codigo_mec . ')') 
                                                       ->load();
                    
                    $param['codigo_validacao'] = $dados_historico[0]->codigo_validacao;
                    
                    $this->onSetDadosHistorico($param);
                }
                else
                {
                    $action_recarregar2 = new TAction([$this, 'onDirecionaRecaptcha'], ['url_amigavel' => $url_amigavel]);            
                    new TMessage('error', "Não foi possível localizar o histórico. Verifique o código de validação e tente novamente.", $action_recarregar2);
                    die;
                } 
            }
            else
            {
                ...
                new TMessage('error', "Não foi possível localizar o histórico. Verifique o código de validação e tente novamente.", $action_recarregar3);
                die;
            }
    ...
    {
        //Chama o formulário ConsultaHistoricoDigitalForm usando a rota    
        TScript::create("window.location.href = 'consultahistorico';");
    }
    ...
    public function onSetDadosHistorico($param)
    {
        try
        {
            ...
            //Traz o histórico que corresponde ao código de validação (deve ser único)
            $count = HistoricoDigital::where('codigo_validacao', '=', $codigo)->count();
            
            if($count == 1)
            {
                $dados_historico = HistoricoDigital::where('codigo_validacao', '=', $codigo)->load();
                    
                $historico = HistoricoDigital::find($dados_historico[0]->id);
                
                if($historico->status_publicacao == 0)
                {  
                    $action_recarregar4 = new TAction([$this, 'onDirecionaRecaptcha'], ['url_amigavel' => $url_amigavel]);            
                    new TMessage('error', "Não foi possível localizar o histórico. Verifique o código de validação e tente novamente.", $action_recarregar4);
                    die;
                }
                else
                { 
            ...
            else
            {
                ...
                new TMessage('error', "Não foi possível localizar o histórico. Verifique o código de validação e tente novamente.", $action_recarregar5);
                die;
            }
    ...
    {
        $html = new THtmlRenderer('app/resources/HistoricoDigital.html');
                
                
        //Informações gerais 
        $dados_gerais = TSession::getValue('dados_gerais');
        $html->enableSection('main', $dados_gerais);
            
            
        //Disciplinas
        $dados_disciplinas = TSession::getValue('disciplinas');             
        $html->enableSection('disciplinas', $dados_disciplinas, TRUE);
        
        $disciplinas_ch_integralizada = TSession::getValue('disciplinas_ch_integralizada'); 
        $html->enableSection('disciplinas_ch_integralizada', $disciplinas_ch_integralizada, TRUE);
        ...
        if(!is_null($dados_atividades))
        {
            $html->enableSection('tabela-atividades');
            $html->enableSection('atividades', $dados_atividades, TRUE);
                        
            //Carga horária total das atividades
            $ch_obrigatoria_atividades = TSession::getValue('ch_obrigatoria_atividades');

            if(!is_null($ch_obrigatoria_atividades))
            {
                $html->enableSection('ch-obrigatoria-atividades', $ch_obrigatoria_atividades, TRUE);
            }
            else
            {
                $html->disableSection('ch-obrigatoria-atividades');
            }
        }
        else
        {
            $html->disableSection('tabela-atividades');
            $html->disableSection('atividades');
            $html->disableSection('ch_obrigatoria_atividades');
        }
        
        $atividades_ch_integralizada = TSession::getValue('atividades_ch_integralizada'); 

        // Verifique se $atividades_ch_integralizada não é NULL e é um array
        if ($atividades_ch_integralizada !== null && is_array($atividades_ch_integralizada)) 
        {
            // Verifique se a chave 'atividades_ch_integralizada' dentro do array é NULL
            if (isset($atividades_ch_integralizada[0]['atividades_ch_integralizada']) && $atividades_ch_integralizada[0]['atividades_ch_integralizada'] === null) 
            {
                $html->disableSection('atividades_ch_integralizada');
            }
        }
        else 
        {
            $html->enableSection('atividades_ch_integralizada', $atividades_ch_integralizada, TRUE);
        }
        ...
        if(!is_null($dados_estagios))
        {
            $html->enableSection('tabela-estagios');
            $html->enableSection('estagios', $dados_estagios, TRUE);
            
            //Carga horária total dos estágios
            $ch_obrigatoria_estagios = TSession::getValue('ch_obrigatoria_estagios');
    
            if(!is_null($ch_obrigatoria_estagios))
            {
                $html->enableSection('ch-obrigatoria-estagios', $ch_obrigatoria_estagios, TRUE);
            }
            else
            {
                $html->disableSection('ch-obrigatoria-estagios');
            }
        } 
        else
        {
            $html->disableSection('tabela-estagios');
            $html->disableSection('estagios');
            $html->disableSection('ch_obrigatoria_estagios');
        }
        
        $estagios_ch_integralizada = TSession::getValue('estagios_ch_integralizada'); 

         // Verifique se $atividades_ch_integralizada não é NULL e é um array
         if ($estagios_ch_integralizada !== null && is_array($estagios_ch_integralizada)) 
         {
             // Verifique se a chave 'atividades_ch_integralizada' dentro do array é NULL
             if (isset($estagios_ch_integralizada[0]['estagios_ch_integralizada']) && $estagios_ch_integralizada[0]['estagios_ch_integralizada'] === null) 
             {
                 $html->disableSection('estagios_ch_integralizada');
             }
         }
         else 
         {
             $html->enableSection('estagios_ch_integralizada', $estagios_ch_integralizada, TRUE);
         }
      
        ///Alterações realizadas no dia 13/06/2024 para atender aos históricos parciais e trasferências no qual não é obrigatório constar atividades complementares e estágio.
            
        //Situações
        $dados_situacoes = TSession::getValue('situacoes');
        $html->enableSection('situacoes', $dados_situacoes, TRUE);
            
        //Observações 
        $dados_adicionais = TSession::getValue('infos_adicionais');
        $html->enableSection('observacoes', $dados_adicionais, TRUE);
      ...
      //  $options->setDebugKeepTemp(true);
       // $options->setLogOutputFile('tmp/dompdf_log.txt');
        ...
        $object->style = "width: 100%; min-height:800px";
           
        $this->notebook->appendPage('RVHE', $object);   
    }
    ...
    public function onVisualizarXML($param)
    {
        try
        {
            ...
            $file = file_get_contents("$historico->caminho_arquivo/$historico->arquivo");
    ...
    public static function onDownloadHistorico($param)
    {
        try
        {
            $id_historico = $param['id_historico'];            
    ...
    public function onShow($param)
    {
        try
        {
            ...
            if($historico)
            {
                $this->panel = new TPanelGroup('HISTÓRICO DIGITAL');
                $this->panel->addHeaderActionLink('Download XML', new TAction([$this, 'onDownloadHistorico'], ['id_historico' => $historico->id]), 'fas:cloud-download-alt blue');
                $this->panel->addHeaderActionLink('Download Representação Visual', new TAction([$this, 'onDownloadRepresentacao'], ['id_historico' => $historico->id]), 'fas:cloud-download-alt blue');
