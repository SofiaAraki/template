...
class MeuCursoViewAluno extends TPage
{
    ...
    {        
        ...
        $repos = VwAlunoMatriculaEtapa::getObjects($criteria6); //MATRICULAS NA UNIDADE ATUAL
        ...
        if($repos) //SE EXISTIREM MATRÍCULAS DESTE USUÁRIO NO SISTEMA
        {
            ...
            {
                $items[$repo->CodCurso] = $repo->NomeCurso; //PEGA O CÓDIGO DA ENTIDADE DE TODAS AS MATRÍCULAS
            }

            //var_dump($items);
            //die;

            $codCursos = array_unique($items); //AGRUPA OS CÓDIGOS DE ENTIDADE IGUAIS
