...
class VwAlunosnotasList extends TPage
{
    ...
    public function __construct()
    {
        ...
        $container->add(new TXMLBreadCrumb('menu.xml', 'VwProfessordisciplinassemestreList'));
    ...
    public static function onSaveInline($param)
    {
        ...
        try
        {
            ...
            { 
                ...
                $result = $conn->query('SELECT * FROM FI_NotasFaltas_Frente WHERE (CodMatriculaEtapa = '.$CodMatriculaEtapa.' AND CodDisciplina = '.$CodDisciplina.' AND Avaliacao = 1)');
                //var_dump($result);
                //die();
                ...
                if ($ID <> NULL){ //necessário para verificar se um dos campos (nota ou faltas) ja foi preenchido em notasfaltasfrente, se já existe faz update
                    ...
                    $conn = TTransaction::get();
                    $result = $conn->query('SELECT * FROM FI_NotasFaltas WHERE (CodMatriculaEtapa = '.$CodMatriculaEtapa.' AND CodDisciplina = '.$CodDisciplina.' AND Avaliacao = 1)');
