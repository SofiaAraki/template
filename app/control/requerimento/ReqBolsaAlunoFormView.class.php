...
class ReqBolsaAlunoFormView extends TPage
{
     protected $form;
    ...
    public function __construct( $param )
    {
        ...
        //Condição para visualizar apenas os requerimentos que o aluno mesmo criou
        $logado = SystemUser::newFromLogin(TSession::getValue('login'));
        
        if($logado->id == $object->system_user_id)
        {
        	$aluno = new ReqBolsaAluno($param['key']);
        }        
        else
        {
            new TMessage('info', 'Acesso negado!');
            die;
        }
        ...
        $label26 = new TLabel('Especificações', '#285097', 12, 'b');
        ...
        //$this->aluno_despesa_list->addQuickColumn('Valor', 'valor', 'left');
        ...
        //$this->form->addHeaderAction('Imprimir', new TAction(['ReqBolsaAlunoFormView', 'onPrint'],['key'=>$aluno->id]), 'far:file-pdf red');
        $this->form->addHeaderAction(('Voltar'),new TAction(array('ReqBolsaAlunoList','onReload')),'far:arrow-alt-circle-left blue');
        ...
        $container->add(new TXMLBreadCrumb('menu.xml', 'ReqBolsaAlunoList'));
    ...
    public function onPrint($param)
    {
        try
        {
            ...
            if ($object)
            {
                ...
                $html = new AdiantiHTMLDocumentParser('app/documents/AlunoDocument.html', 'A4', 'portrait');
